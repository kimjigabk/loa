...
class BossEdit2 extends React.Component {
  ...
  componentDidMount() {
    const pb = populateBoss(this.props.bossProgress, 2);
    const {
      normal,
      hard,
      gateway,
  ...
  renderDifficulty() {
    ...
    return (
      <div>
        ...
        {this.props.editngCharitemLevel >= 1460 && (
          <button
  ...
  renderGateway() {
    return (
      <div>
        ...
        {this.state.gateway && this.props.editngCharitemLevel >= 1460 && (
          <div style={{ paddingTop: "10px" }}>
            <button
  ...
  renderGold() {
    return <div>Gold Earned: {calculateGold(this.state, 2)}</div>;
  }
  modalContent() {
    return (
      <div>
  ...
  render() {
    return (
      <div>
        <Modal
          title={`Edit Boss2 for: ${this.props.editingCharName}`}
...
export default connect(mapStateToProps, { updateBossProgress })(BossEdit2);
